...
public:
    ...
    SharedPtr<Node> coneCamNode_;
    SharedPtr<Node> coneGimbal_;
    RigidBody* coneRigidBody;

    const float g0 = 9.8; // m/s^2
    const float dryMass = 22200; // kilograms
    float fuelMass = 410000; // kilograms
    const float maxThrust = 7600000; // Newtons
    const float Isp = 275*g0; // m/s
    float thrustMultiplier = 100; // fraction
    ...
    */
    virtual void Start()
    {
        ...
        scene_->LoadXML(*cache->GetFile("lander_scene.xml"));
        ...
        coneNode_=scene_->GetChild("cone_lander", true);
        coneCamNode_=scene_->GetChild("cone_camera", true);
        coneGimbal_=coneNode_->GetChild("cone_gimbal", true);
        Camera* coneCam = coneCamNode_->GetComponent<Camera>(); // this works now
        ...
        SharedPtr<Viewport> viewport(new Viewport(context_,scene_,coneCam));
        renderer->SetViewport(0,viewport);

        coneRigidBody = coneNode_->GetComponent<RigidBody>();
        coneRigidBody->SetMass(dryMass+fuelMass);
    ...
    */
    void HandleKeyDown(StringHash eventType,VariantMap& eventData)
    {
        ...
        if(key==KEY_SPACE){
            //coneNode_->GetComponent<RigidBody>()->ApplyForce(*(new Vector3((float)0,(float)1000000,(float)0)));
        }
    }
    ...
    */
    void HandleUpdate(StringHash eventType,VariantMap& eventData)
    {
        ...
        {
            std::string str;
            str.append("Keys: tab = toggle mouse, Esc = quit.\n");
            {
        ...
        if(input->GetKeyDown('W')) {
            ...
            coneRigidBody->ApplyTorque(Vector3::UP.CrossProduct(Vector3::FORWARD*100000));
        }
        if(input->GetKeyDown('S')) {
            //cameraNode_->Translate(Vector3(0,0,-1)*MOVE_SPEED*timeStep);
            coneRigidBody->ApplyTorque(Vector3::UP.CrossProduct(Vector3::BACK*100000));
        }
        if(input->GetKeyDown('A')) {
            //cameraNode_->Translate(Vector3(-1,0,0)*MOVE_SPEED*timeStep);
            coneRigidBody->ApplyTorque(Vector3::UP.CrossProduct(Vector3::LEFT*100000));
        }
        if(input->GetKeyDown('D')) {
            //cameraNode_->Translate(Vector3( 1,0,0)*MOVE_SPEED*timeStep);
            coneRigidBody->ApplyTorque(Vector3::UP.CrossProduct(Vector3::RIGHT*100000));
        }
        ...
        if(input->GetKeyDown(' '))
        {
            // Use the engine
            if (fuelMass>0) {
                fuelMass -= (maxThrust * thrustMultiplier) / (Isp * 10000);
                Vector3 force = coneNode_->GetComponent<RigidBody>()->GetRotation() *
                                Vector3::UP * maxThrust * thrustMultiplier;
                coneRigidBody->ApplyForce(force * timeStep);
                coneRigidBody->SetMass(fuelMass + dryMass);
            }
        }
        ...
        if(!GetSubsystem<Input>()->IsMouseVisible())
        {
            ...
            coneGimbal_->SetDirection(Vector3::FORWARD);
            coneGimbal_->Yaw(yaw_);
            coneGimbal_->Pitch(pitch_);
        }
    }
    /**
